...
retry timeout 20 add-apt-repository -y \
...
if [ -z "$storage_mount" ]; then
    ...
    echo "ExecStart=/usr/bin/dockerd --data-root $storage_mount/docker -H fd://" >> /etc/systemd/system/docker.service.d/docker.root.conf
